...
item_NODE createi(item_NODE ite, int rr){
    if(ite==NULL){
        ...
        scanf("%d", ite->stock);
        ite->item_key = 5467;
        printf("This is your item security key. Do not share it with anyone.\nSecurity key:%d",ite->item_key);
...
void preorderi(item_NODE ite){
    if(ite!=NULL){
        printf("%s\t\t%d\t\t%d",ite->item_name, ite->item_key, ite->price);
        preorderi(ite->left);
        preorderi(ite->right);
    }
}
